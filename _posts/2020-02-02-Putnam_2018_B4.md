...
Thus we see that if we can prove the sequence of Fibonacci numbers $$\lbrace F_k \rbrace_{k \geq 0} \pmod{4F_n}$$ is periodic, then we will have also proven that the sequence $$\lbrace x_i \rbrace$$ is periodic as well.
...
Thus the sequence $$\lbrace F_k \rbrace \pmod{4F_n}$$ is fully periodic, meaning there exists an integer $$T$$ such that

\\[ x_{n+T} = \cos (F_{n+T}\theta) = \cos (F_n\theta) = x_n \\]
